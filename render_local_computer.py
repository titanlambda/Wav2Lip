...
def load_data():
	...
	print("###### Model loaded!")
	print("###### Loading video")
	video_stream = cv2.VideoCapture("./video_sample/silent.mp4")
	...
	while 1:
		...
		return model, full_frames, fps


def render(model, full_frames, fps, audio_file):
	...
	command = './ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(audio_file, 'temp/result.avi', output_video_path)
	subprocess.call(command, shell=True)

def process(text):
	generate_audio_tts(text, audio_path)
	model, full_frames, fps = load_data()
	render(model, full_frames, fps, audio_path)
